...
import { FolderTree, Plus, Edit2, Package, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-[#EFE3E6]">
        <div>
          ...
          <p className="text-xs text-[#75686C]">
            Kelompokkan barang idaman berdasarkan kategori minat. Klik kartu untuk melihat daftar produk di dalamnya.
          </p>
        </div>
        <button
          type="button"
      ...
      {categories.length === 0 ? (
        <div className="bg-white border border-[#EFE3E6] rounded-[4px] p-8 text-center max-w-sm mx-auto my-6 space-y-2">
          <FolderTree className="w-8 h-8 text-[#75686C]/50 mx-auto stroke-1" />
          <p className="text-xs text-[#2C2528] font-semibold">Belum Ada Kategori</p>
          <p className="text-[11px] text-[#75686C]">Buat kategori pertama Anda untuk mengelompokkan wishlist.</p>
          <button
            type="button"
            onClick={onOpenAddModal}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-3.5 sm:gap-4 max-[480px]:gap-2.5">
          {categories.map((category) => {
            ...
            const totalPrice = catProducts.reduce((sum, p) => sum + p.price, 0);
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2.5">
                      <div
                        ...
                        style={{
                          backgroundColor: `${category.color || '#E87A90'}15`,
                  ...
                  {category.description && (
                    <p className="text-xs text-[#75686C] line-clamp-2 leading-relaxed mb-3">
                      {category.description}
                    </p>
                  )}
                </div>
                ...
                <div className="pt-3 border-t border-[#F6ECEE] flex items-center justify-between text-xs">
                  <div className="space-y-0.5">
                    <span className="text-[11px] text-[#75686C]">
                      <strong className="font-sora text-[#2C2528]">{catProducts.length}</strong> produk
                    </span>
